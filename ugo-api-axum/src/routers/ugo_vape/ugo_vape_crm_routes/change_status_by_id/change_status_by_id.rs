...
use crate::routers::ugo_vape::ugo_vape_crm_routes::change_status_by_id::new_status_notification_ugo::new_status_notification_ugo;
...
pub async fn change_status_by_id(pool : Extension<Arc<Mutex<PooledConn>>>, Json(body) : Json<ChangeOrderStatusBody>) -> impl IntoResponse {
    match extract_u32(body.order_id) {
        Ok(id) => {
            ...
            match change_status_by_id_sql(&mut unlocked, id, body.new_status.clone()) {
                Ok(_) => {
                    match new_status_notification_ugo(id, body.new_status).await {
                        Ok(()) => {
