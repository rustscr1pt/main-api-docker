...
use crate::routers::tg_notification_routes::new_note_ugo::get_last_note_sql_ugo::get_last_note_sql_ugo;
use crate::structs::extension_structs::SQLAndTelegramWebExtension;
use crate::structs::structs::TelegramRequest;

pub async fn new_note_ugo(main_actor : Extension<SQLAndTelegramWebExtension>, Json(body) : Json<TelegramRequest>) -> impl IntoResponse {
    if body.key == r#"XP{B3edA"s$i4im78u-Jt3BkTBH]%("# {
        match get_last_note_sql_ugo(Arc::clone(&main_actor.arc_sql), Arc::clone(&main_actor.telegram_bot)).await {
            Ok(()) => {
